...
var (
	DefaultProtoPackageName = "pb"
	DefaultProtoServiceName = "entpb"
	DefaultGoPackageName    = "./pb"
	IDFieldNumber           = 1
)
...
func SetDefaultProtoServiceName(name string) {
	DefaultProtoServiceName = name
}
...
func (a *Adapter) parse() error {
	...
	var allServiceName = make(map[string][]string)
	...
	for _, genType := range a.graph.Nodes {
		...
		if _, ok := protoPackages[protoPkg]; !ok {
			...
			EmptyMessage := &descriptorpb.DescriptorProto{
				Name:  strptr("Empty"),
				Field: []*descriptorpb.FieldDescriptorProto{},
			}

			BaseIDResp := &descriptorpb.DescriptorProto{
				Name: strptr("BaseIDResp"),
				Field: []*descriptorpb.FieldDescriptorProto{
					{
						Name:   strptr("id"),
						Number: int32ptr(1),
			...
			BaseUUIDResp := &descriptorpb.DescriptorProto{
				Name: strptr("BaseUUIDResp"),
				Field: []*descriptorpb.FieldDescriptorProto{
					{
						Name:   strptr("id"),
						Number: int32ptr(1),
			...
			protoPackages[protoPkg].MessageType = append(protoPackages[protoPkg].MessageType,
				EmptyMessage,
				BaseIDResp,
				BaseUUIDResp,
			)
		...
		if svcAnnotation.Generate {
			...
			var methods []string
			for _, m := range svcResources.svc.Method {
				methods = append(methods, m.GetName())
			}
			allServiceName[svcResources.svc.GetName()] = methods
		}
	}
	...
	for dp, fd := range descriptors {
		...
		allInOneServiceBuilder := builder.NewService(DefaultProtoServiceName)

		for svr, methods := range allServiceName {
			hackService := fbuild.GetService(svr)

			for _, m := range methods {
				method := hackService.GetMethod(m)
				groupName := strings.Replace(svr, "Service", "", 1)
				method.SetComments(builder.Comments{
					LeadingComment: "group:" + groupName,
				})
				allInOneServiceBuilder.AddMethod(method)
			}

			fbuild.RemoveService(svr)
		}

		fbuild.AddService(allInOneServiceBuilder)
...
func extractProtoTypeOptionalDetails(f *gen.Field) (fieldType, error) {
	if f.Type.Type == field.TypeJSON {
		return extractJSONDetails(f)
	}
	cfg, ok := typeMap[f.Type.Type]
